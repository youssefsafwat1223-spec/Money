...
- **Last updated:** **Phase 7 Batch 2 (in progress) — performance: query/provider/
  startup/rendering/reporting/background — 2026-08-07.** Established a structural (not
...
| MALI-029 | Med | C | P7 | **Partial · Locally verified** | **P7-B2.** Scoped invalidation DONE: `AppDatabase.tableWriteStream` emits each write's target table; `scopedRevisionProvider(family)` + `financialRevisionProvider` (exclude-operational) replace the global tick for reports/budgets/transaction-list/dashboard; unrelated write→0 rebuilds, relevant/display-dep→exactly 1, 100-burst→≤2, operational→0 (`scoped_invalidation_test.dart`). Pull-batching (measured): CaptureSyncService accounts prefetch (8 captures→1 read) + LedgerSync account resolution (25 rows→1 read). Cadence DONE: adaptive backoff (30s→5min) + request coalescing (SyncCadence/SyncCoalescer, 7 tests). Search debounced. **Remaining:** SenderBankMapping/PlanningPull/etc. batching + repo saveTransaction category re-resolution + rendering/getPage filter. See `PHASE_7_PERFORMANCE_CONTRACT.md`. |
| MALI-030 | Med | C | P7 | **Not started (audited)** | P7-B2 audited: `_largestTransactions`/appendix/CSV/backup-snapshot materialize the whole table; v3 AES-GCM one-shot is an unavoidable-within-v3 crypto-library residual (no v4 this batch). Fix pending. See `PHASE_7_PERFORMANCE_CONTRACT.md` §5. |
