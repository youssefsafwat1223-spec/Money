-- Restores the exact 0040 function definition (ON CONFLICT target already fixed,
-- but paid_count = excluded.paid_count still present in the conflict-recovery SET
-- list). Do not roll back unless a new, unrelated regression is suspected to
-- originate from 0041 — the pre-0041 version silently overwrites paid_count with
-- p_paid_count (typically null/stale) on every retry or second call.
