...
> # ⛔ SUPERSEDED IN PART — DO NOT RELY ON THIS DOCUMENT'S VERDICT
>
> **A cross-model technical audit on 2026-08-23 (HEAD `12f36726`) found 4 CRITICAL and ~24 HIGH
> defects that this package did not detect.** Three of the CRITICALs silently corrupt or fail to
> delete user financial and personal data, and two are reachable from ordinary use:
>
> - **C-1** — editing a budget/goal/bill silently rewrites its stored amount (a 1500.50 budget
>   becomes 1501 on an untouched Save).
> - **C-2** — the advertised "Export all Qirsh data" ZIP corrupts canonical planning money on import,
>   unrecoverably.
> - **C-3** — migration `0083` silently reverted `purge_user_data()`, so account deletion leaves
>   identity-bearing rows behind while reporting erasure complete.
> - **C-4** — an unvalidated AdMob application id crashes the app at launch for every user.
>
> Authoritative record: **`docs/FINAL_CROSS_MODEL_AUDIT_RECONCILIATION.md`**. Where the two
> disagree, the audit wins. Individual corrections are marked inline below as
> **OPEN / UNDER REMEDIATION**. Sections not yet re-verified against the audit should be treated as
> **unconfirmed**, not as passed.
>
...
| A4 | Android compile gate now **auto-triggers** in `backend-and-quality-gates` on push/PR to protected + `feat/*` branches (Batch 15, H-16), and every release workflow runs `tools/ci_gates.sh` (strict) before signing/build (H-15). Proven in CI **configuration** (17 structural contract tests); **not yet verified on a real remote run** (no push authorised). | **CONFIG_READY — PENDING_REMOTE_EXECUTION (H-16/H-15)** |
...
| **Skip nonce checks = ON — GOOGLE ONLY** | The native Google SDK hashes its own nonce inside the `id_token` and never exposes the raw value, so Supabase cannot compare it; without this, Google sign-in fails. `skip_nonce_check` is a **per-provider** setting (`[auth.external.<provider>]`), so this does NOT apply to Apple |
| **Skip nonce checks = OFF for APPLE** | ⚠️ **Corrected 2026-08-23 (audit H-9).** The app now generates a per-attempt nonce, sends `SHA-256(raw)` to Apple and the raw value to the token exchange. Turning this ON for Apple would stop the backend verifying that binding and make the fix cosmetic — it must stay **OFF** |
...
### 20.3 Android CI compile gate (R8C)

The defect above survived because Android was compiled **only** by `android-release` — on release
day, behind the real signing inputs. A compile error would therefore surface at the worst possible
moment, and only *after* the signing step had already run.

`backend-and-quality-gates` (the canonical pre-release workflow) now runs an **Android compile
gate**: `flutter build apk --debug`, pinned to Flutter 3.44.2 + **JDK 17**, installing the pinned
NDK `28.2.13676358`. A debug APK is the cheapest artifact that still exercises Gradle 9.1.0, AGP
9.0.1, Kotlin 2.3.20, javac, `GeneratedPluginRegistrant`, native plugins, sqlite3mc via the NDK and
manifest merge — and it needs **no signing key, no production AdMob ids and no backend**. The gate
asserts the vendored `file_picker` is wired (so resolution can never silently fall back to pub.dev
and lose the patch) and checks the APK actually exists rather than trusting the exit code. It is
hard-failing: no `allow_failure`, no `ignore_failure`, no `|| true`.

**Boundary decision.** The compile deliberately does **not** live in `tools/ci_gates.sh`: that suite
is the portable local verification path, and forcing every invocation to download a ~3 GB NDK would
destroy that portability. The canonical CI therefore runs two mandatory stages — the portable gates,
then the Android compile.

> **⚠️ CORRECTION HISTORY (finding O-2 / H-16).**
>
> **2026-08-23 (audit):** the earlier "mandatory / ordinary pre-release path" wording was
> false — `codemagic.yaml` had **no `triggering:` section**, so `backend-and-quality-gates`
> was manual-start only and the Android compile never ran on push/PR.
>
> **2026-08-23 (Batch 15 — CONFIG_READY / PENDING_REMOTE_EXECUTION):** fixed in configuration.
> `backend-and-quality-gates` now has a `triggering:` block (push + pull_request on
> `main`/`master`/`develop`/`feat/*`, docs-excluded) and runs `tools/ci_gates.sh` + the fatal
> Android compile. Every release workflow now runs `ci_gates.sh` (strict) BEFORE signing/build
> (H-15). Structural contract tests (`ci_release_gate_authority_test.dart`, 17 cases) assert
> the triggers, the gate-before-artifact ordering, and that release workflows stay manual.
> **Still not proven on a real remote run** — no push is authorised, so REMOTE_CI is
> PENDING_PUSH. Do not claim "mandatory Android CI gate remotely verified" until a pushed run
> succeeds.

`android-release` also gained a **signer inspection** step between building the AAB and shredding
the keystore: it fails if the artifact is unsigned or carries the Android **debug** certificate.

> **⚠️ CORRECTION (2026-08-23, cross-model audit — finding O-1/H-17). STATUS: FIXED, EVIDENCE LOCAL.**
>
> The keystore-shredding step introduced here shipped with an **unterminated shell quote**
> (`echo "Materialised keystore removed.`). Under `set -eu` the shell failed to *parse* the block,
> so `android-release` could **never complete successfully**, and the `rm -f` never ran — leaving
> the materialised upload keystore in the runner workspace until teardown.
>
> Found independently by two audit reviewers. The quote is now closed, verified by extracting all
> `script: |` scalars from `codemagic.yaml` and running `bash -n` over each (0 failures); the check
> was first proven to catch the original defect. Remote CI execution remains unverified.
>
> Root cause of the miss: the contract tests asserted `rm -f` appeared as **text** and never
> validated the scalar as shell.
Responsibilities stay split — the compile gate needs no secrets; the release workflow still demands
the explicit A2 signing inputs. GitHub Actions stays **verification-only**; no release secret is
duplicated into it.

**17 static contract tests** pin all of the above (compile present, hard-failing, artifact checked,
Flutter/JDK/NDK pinned, vendored fork wired, companion deps unmigrated, no signing/AdMob coupling,
release ordering, `ci_gates.sh` kept portable).

⚠️ **Not yet executed remotely.** Git push is not authorised, so this configuration is
**READY** but **REMOTE_CI_EXECUTION — PENDING_PUSH**. It must not be recorded as green until it
actually runs on Codemagic.
...
| BACKUP | **READY** |
| CI | **CONFIG_READY — PENDING_REMOTE_EXECUTION (Batch 15)** — canonical `ci_gates.sh` now runs GREEN in **strict** mode (`failed:0 unavailable:0 external:3`; the deps-policy gap on the vendored `file_picker` fork was fixed). H-15: every release workflow runs `ci_gates.sh` (strict) before signing/build/submit. H-16: `backend-and-quality-gates` now auto-triggers on push/PR with the fatal Android compile. CI-02 closed (strict mode makes a tool-missing stage FATAL). **All proven in configuration + local run only — a real pushed remote run is PENDING and NOT verified.** |
