// Referral & Ads Phase R2 — reverse a QUALIFIED referral (fraud case B, spec §8).
// The immutable history is NOT rewritten: the referral is marked reversed
// (qualified_at retained) and a reversal row is appended to the ledger.
// Reversal does NOT by itself shorten an entitlement or decrement progress —
// those are separate, explicit Admin actions. Verb-only export.
...
export async function POST(req: NextRequest) {
  ...
  const { data, error } = await supabase.rpc("admin_reverse_referral", {
  ...
  if (error) {
    console.error("[referrals:reverse]", error);
