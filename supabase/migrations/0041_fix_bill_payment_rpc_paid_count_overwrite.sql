-- Same-day fix for a bug surfaced by 0040's live-testing (present since 0039, but
-- unreachable before 0040 since every call failed outright with 42P10):
-- create_subscription_and_record_payment's subscription upsert's
-- ON CONFLICT DO UPDATE SET included `paid_count = excluded.paid_count`, so any
-- retry (or any second call for the same local_id) overwrote the correct,
-- payment-logic-maintained paid_count with whatever p_paid_count happened to be
-- passed at that call (defaults to null, and is not authoritative — the later
-- `elsif subscription_row.type = 'installment' then update ... paid_count` block
-- is the sole owner of that value once a subscription exists).
--
-- Fix: stop touching paid_count in the conflict-recovery SET list entirely. On
-- first INSERT (no conflict), paid_count is still initialized from p_paid_count
-- exactly as before — unchanged. On any conflict (idempotent retry, or a future
-- legitimate "update subscription details" call through this RPC), paid_count is
-- now left untouched, preserving whatever the payment logic already established.
-- This is the only change in this migration: one line removed from the SET list.
-- Every parameter, the return shape, SECURITY INVOKER, search_path, and grants
-- are unchanged from 0040.
