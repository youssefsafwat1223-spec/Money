...
| **`admin/.env.local` points at a FORBIDDEN legacy project** | MEDIUM | On this machine `admin/.env.local` sets `SUPABASE_URL` to `vrombzdgwqjjiijbidqb` — one of the three refs under the standing ZERO CONTACT rule — not the linked project `rjwphwsefnuotpbtuycf`. The file is gitignored and has never been committed, so this is a local-workstation hazard, not a repository one: anyone running the admin panel locally, or any tool that sources it, silently targets the wrong project. Found 2026-09-04 when a row-count query sourced it and contacted that host (one read-only GET, HTTP 404, no data returned, nothing written). **Repoint or delete the file.** The lesson generalises: the project-ref guard was only being applied to Supabase CLI calls, and any credential source outside the CLI bypassed it. |
| **Workers rollback host serves stale legal copy** | LOW–MEDIUM | `qirsh-legal.albaraai-dev.workers.dev` still serves the pre-2026-09-04 text, including the false "and notification messages" claim. It is deliberately untouched: it is the rollback host and builds already in users' hands have its URL compiled in. **Known non-blocking failback risk** — failing back restores availability but *reinstates a false capability claim* on a document the Play declaration attaches. If failback is ever needed, redeploy the corrected content there first, or accept and document the exposure. Recorded 2026-09-04; that host must not be changed as part of unrelated work. |
