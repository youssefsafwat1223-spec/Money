...
| Supabase migrations | **0001…0098** active (0099 deferred), 42 rollback files, lint PASS |
| Edge Functions | 28 |
...
lint PASS. 0099 is deferred out of the active set (`supabase/deferred/`).
...
**0093–0098 are source-only and must not be assumed deployed. 0099 is DEFERRED** to `supabase/deferred/` — it would enable report-export telemetry with no flag able to stop it. Every feature
...
1. `record_metric` has no consent gate (owner-bound, allowlisted, rate-limited
   server-side; sends a key plus a coarse dimension). Deferred migration `0099` finally
