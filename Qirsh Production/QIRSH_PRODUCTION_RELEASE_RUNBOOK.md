...
**Live at `https://qirsh.site`** (canonical, TLS, on the Qirsh VPS).

This is the **canonical production legal host**: the owned domain `qirsh.site`,
served by nginx on the Qirsh VPS with a Let's Encrypt certificate, HSTS, and
renewal proven by `certbot renew --dry-run`. `/privacy` and `/terms` return
**200 directly** — no trailing-slash redirect.

Generated by `tools/build_site.py` and deployed to `/var/www/qirsh-site`.
Architecture: [`14_Website_Hosting/`](14_Website_Hosting/).

The previous host `qirsh-legal.albaraai-dev.workers.dev` was an approved
temporary production host. It **stays live as the rollback** — shipped builds
have its URL compiled in. See
[`04_Legal/domain_status.md`](04_Legal/domain_status.md).

<details><summary>Original hosting options, kept for reference</summary>
...
The legal site is live at `https://qirsh.site` — the **canonical production host**
([`04_Legal/domain_status.md`](04_Legal/domain_status.md)) — and it is the
**built-in default** in `legal_urls.dart`. The former Workers host stays live
as the rollback.
...
```
LEGAL_BASE_URL=https://qirsh.site
```
