...
| MALI-029 | Med | C | P7 | **Partial · Locally verified** | **P7-B2.** Scoped invalidation DONE: `AppDatabase.tableWriteStream` emits each write's target table; `scopedRevisionProvider(family)` + `financialRevisionProvider` (exclude-operational) replace the global tick for reports/budgets/transaction-list/dashboard; unrelated write→0 rebuilds, relevant/display-dep→exactly 1, 100-burst→≤2, operational→0 (`scoped_invalidation_test.dart`). Pull-batching (measured): CaptureSyncService accounts prefetch (8 captures→1 read) + LedgerSync account resolution (25 rows→1 read). Cadence DONE: adaptive backoff (30s→5min) + request coalescing (SyncCadence/SyncCoalescer, 7 tests). Search debounced. SenderBankMapping push batched (25→1 upsert, per-row fallback). **Production-reachable inventory** (pump=_runLedgerSyncBody + bootstrap): CaptureSync✓, LedgerSync✓, SenderBankMapping✓ batched; PlanningPull (_ensureMerchant/category/account per row), PlanningChildSync, AccountsPull per-row lookups, shared repo saveTransaction category re-resolution — STILL per-row, NOT yet batched. Cadence: adaptive backoff+coalescing✓; offline-signal detection NOT yet. Backfills = one-time startup migrations (StartupSyncReconcile). **MALI-029 NOT complete** — remaining active O(rows) loops above still to fix. rendering/getPage = B2-C. See `PHASE_7_PERFORMANCE_CONTRACT.md`. |
...
| MALI-038 | Low | C+T | P7 | **Partial · Locally verified** | **P7-B2.** Asset portion DONE: removed 8.1 MB unreferenced assets (16.7→8.6 MB), `asset_budget_test.dart` enforces ≤1 MiB/file + ~11 MiB total. **Font portion PENDING a product decision** — production typography uses GoogleFonts runtime fetch (breaks offline); Alexandria isn't bundled and can't be fetched offline; switching to the vendored IBM Plex Sans Arabic is offline-correct but a visible typeface change. See `PHASE_7_PERFORMANCE_CONTRACT.md` §8. |
