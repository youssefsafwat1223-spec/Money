...
- **Last updated:** **Phase 7 Batch 4 — hardening (2026-08-10): privacy surface,
  ...
  guard 6/6; offline deps policy; iOS packaging PROVENANCE-verified against a fresh
  stamped sim build). B4-8 fixed a closure blocker — the iOS stage had counted a
  stale pre-`fe8c9267` Runner.app as current proof; now provenance-gated
  (stale→UNAVAILABLE). Remaining P7 evidence is external (signed builds,
  physical devices, live backend/APNs, hosted CI matrix, CVE feed). Canonical
  brand Qirsh/قِرش; bundle id unchanged. See `PHASE_7_BATCH_4_CLOSURE.md`.
  _Prior:_ **Phase 7 Batch 3 — CLOSURE (2026-08-10): MALI-034 +
  MALI-040 closed; financial authority, DB lifecycle ownership, legacy recovery,
  and architecture boundaries reconciled and guarded.** Batch-3 closure gate
  first-attempt green on committed tree `ac3258aa` (11/11 mandatory; bulk 1583 +
  crypto 24; arch guard 6/6). Deps: nothing removable; residual Supabase-financial
  escape-hatch sweep clean; retained backfills safe mid-run (entry-admission +
  preflight ownership + server RLS `WITH CHECK` + fail-closed); `financial_cache_health`
  = historical recovery only (no writer sets dirty=1). See `PHASE_7_BATCH_3_CLOSURE.md`.
  _Prior:_ **Phase 7 Batch 3 — MALI-040: test DB/executor ownership +
  a production `close()` lifecycle defect fixed — 2026-08-09.** `AppDatabase.close()`
  never called `super.close()`, so Drift's `streamQueries` was never disposed and
  its open-db counter never decremented (632 spurious "multiple databases"
  warnings; a real stream-teardown leak on sign-out/restore). Fixed (`f33d6e58`);
  warnings 632→57 (all legitimate concurrent independent-executor tests); all 4
  `dontWarnAboutMultipleDatabases` suppressions removed; the arch guard forbids
  reintroduction; gate first-attempt green on `54e53a60`. See MALI-040 row +
  `PHASE_7_MALI_040_CLOSURE.md`. _Prior:_ **MALI-034: Supabase-primary financial
...
| MALI-040 | Low | T+C | P7 | **Code complete · Locally verified — deterministic test DB/executor ownership; all test DBs close explicitly; multi-db warning suppression removed** | **P7-B3 (2026-08-09) — test DB/executor ownership + a PRODUCTION close() defect fixed.** §1 inventory (111 DB-owning test files): 0 leave a DB unclosed, 0 share a QueryExecutor across wrappers (each open uses a fresh inline `NativeDatabase.memory()`/`File()`), 4 files suppressed the warning. **Root cause of the 632 "multiple databases" warnings = a production bug, not a test defect:** `AppDatabase.close()` overrode `GeneratedDatabase.close()` but `_close()` called `executor.close()` directly and never `super.close()`, so Drift's `streamQueries` was never disposed on close (a real teardown leak — `.watch*` used in 60 lib files, closed+reopened on sign-out/account-switch/restore) and Drift's open-db counter never decremented (the warning source). **Fix `f33d6e58` (`fix(db): close Drift database lifecycle correctly`):** `_close()` now calls `super.close()` in place of the bare `executor.close()` — each AppDatabase owns its own executor so it closes exactly once; Phase-6 lease/lifecycle/idempotency unchanged. Regression `database_close_lifecycle_test.dart` (fails on old code, no suppression). Result: warnings **632→57**, full bulk suite green (1582). §3 all 4 `dontWarnAboutMultipleDatabases` suppressions removed (prod=0/tests=0); each formerly-suppressed file now 0 warnings. §5 the 57 residual warnings are all legitimate CONCURRENT independent-executor tests (source+target / db+dbB / dbProd+dbFramework), each closed — no shared-executor duplicate-ownership defect exists. §8 `tools/check_arch_guard.sh` now forbids the suppression anywhere (self-tested); global fail-on-warning is not practical (legitimate concurrent tests), so: zero suppression + deterministic ownership + guard. Prior P7-B2 `database_lease_test` isolate-kill hardening retained. Full narrative: `PHASE_7_MALI_040_CLOSURE.md`. **Closure gate:** `ci_gates.sh` ran once from committed clean tree `54e53a60`, **first attempt green** — 11/11 mandatory, 0 failed, 0 unavailable (bulk 1582 + serialized Argon2 crypto 24; arch guard 6/6 incl. the MALI-040 suppression check; skip manifest satisfied). Commits: `f33d6e58` (production close fix) + `54e53a60` (suppressions/guard/docs). **Bookkeeping:** executable closure verified at `54e53a60`; documentation-only closure record at `f86f1629`. Not pushed. |
...
| MALI-042 | Low | T | P7 | **Code complete · Locally verified (B4-4 — all-functions Deno in the gate)** | Edge unit isolation (MALI-066n) |
| MALI-043 | Low | P+D | P7 | **Code complete · LV — brand=Qirsh/قِرش (Decision 1); unjustified NSLocation removed (B4-1/B4-7); current packaged artifact provenance-verified via fresh sim build (B4-8); SIGNED archive + store review external** | canonical brand name (Mali vs Qirsh) |
...
| MALI-077n | Low | C+P | P7 | **Code complete · LV (B4-6 — codemagic email/comment, dead Dart consume API deleted; Kotlin com/example path documented cosmetic; native handler removal external)** | ops lows (keystore name, email, dead API, package path) |
...
| P7 CI/test/arch/docs | 066n,067n,029,030,034,035,037,038,040,041,042,043,077n,036-limits,021-deadfile | **Batch-4 Code complete · LV (authoritative closure gate first-attempt green on `82cd7e6f`, 13/13 mandatory, bulk 1589 + crypto 24).** _(B4-8 closure blocker: the iOS packaging stage had counted a STALE Aug-5 Runner.app as current proof; fixed with a provenance gate — `ios_input_sha` sidecar; stale→UNAVAILABLE, never a pass — and re-verified against a FRESH stamped sim build proving NSLocation absent + قرش. Supersedes the earlier `11727d81` run.)_ B4 hardening closed the remaining P7 local findings: MALI-035 (docs truthfulness), MALI-037 (offline deps-policy gate; CVE external), MALI-039 (already·LV, hostile-id regression added), MALI-042 (all-functions Deno in gate), MALI-043 (Qirsh brand + NSLocation removed), MALI-044 (source-remediated by undeployed 0072; deployment external), MALI-066n (CI coverage), MALI-067n (test integrity), MALI-077n (ops lows). MALI-021 superseded; MALI-036 external; MALI-026 deferred (P8). See `PHASE_7_BATCH_4_CLOSURE.md`. **Prior: Batch-3 APPROVED · CLOSED (2026-08-10) — "Code complete — locally verified; financial authority, database lifecycle ownership, legacy recovery, and architecture boundaries are reconciled and guarded." Executable closure `ac3258aa`; docs-only record `f3cab084`.** Closure gate first-attempt green on `ac3258aa`. MALI-034 (Supabase-primary financial authority retirement) CLOSED — exec `a72f1b11`; MALI-040 (test DB/executor ownership + production `close()` defect) CLOSED — exec `54e53a60`. Both behind `tools/check_arch_guard.sh` (6 checks). Batch-3 reconciliation (deps: nothing removable; residual sweep clean; retained-backfill mid-run safety via entry-admission+preflight+RLS+fail-closed; financial_cache_health = historical recovery only; SyncGate consistency; randomized-order evidence) in `PHASE_7_BATCH_3_CLOSURE.md`. Executable Batch-3 closure verified at `ac3258aa`. MALI-041 already Code-complete·LV. Remaining P7 items (035/037/038/042/043/066n/067n/077n/…) not started. |
