...
| MALI-013 (Android capture) | ◑ Implemented · locally verified · ext-pending | `e9c9ec57` | **Implemented:** durable native queue (`DurableCaptureQueue`, SharedPreferences-backed, enqueue/peek/ack, idempotent, bounded, corruption-safe) replaces the process-memory list so shared messages survive process death; opt-in `SmsCaptureReceiver` (financial pre-filter → same queue) + `CaptureSettings` (two-key: declared+granted+enabled, OFF by default); honest capability API (`captureCapabilities` — notification perm never reported as SMS) replacing the misleading `hasSmsPermission`; manifest is **Play-safe by default** (RECEIVE_SMS + receiver commented, documented). **Locally verified:** 6 Dart capability-model tests; analyze clean; full suite. **External verification pending:** Android build/manifest-merge, runtime permission dialog, real SMS receiver + durable-queue behavior on device (process-death/reboot), and a Google Play Permissions Declaration + prominent disclosure. **NOT closed.** |
...
| 7 | Card cloud rollout (MALI-017) | Live Supabase + client build | Migration 0064 deployed; capability advertised; flip `kUserCardsCloudV2=true`; unassigned card + design fields round-trip to a second device | Card/customization disappears on reinstall/second device | Keep flag false (client guard warns before loss); re-enable once 0064 verified live |
| 8 | CI workflows run + old-client compat (MALI-036) | Codemagic + staging Supabase | `android-release` + `backend-and-quality-gates` run green on Codemagic; migrations apply on staging; an old client build works against the new schema and vice-versa | Green CI ships a broken Android/backend combo; a migration breaks an old client | Roll back the migration; gate the client on a min-schema-version check |
| 9 | Android capture on device (MALI-013) | Android SDK + device | App builds; share-to-Mali enqueues to the durable queue and survives process death/reboot; peek/ack import round-trips; capability states report correctly | Shared messages lost on process death; wrong permission state shown | Durable queue is the fix; verify queue survives kill/reboot on device |
| 10 | Android auto-SMS rollout (MALI-013) | Google Play + device | Play Permissions Declaration approved for SMS money-management + prominent disclosure added; uncomment RECEIVE_SMS + `SmsCaptureReceiver` (or a dedicated flavor); opt-in flow grants + enables; only financial SMS enqueue; denial degrades to share | App rejected/removed for undeclared RECEIVE_SMS; unrelated SMS captured | Keep Play-safe build (perm commented); ship auto-SMS only after approval |
| 11 | Android durable-queue unit tests (MALI-013) | Android/JVM test harness | Wire JUnit/Robolectric test source set; queue tests (survives recreation, peek≠delete, ack-only-matching, idempotent, malformed-safe) run green | Queue regressions ship unverified | Logic is written; add the Android test harness (none exists yet) and run |
| 12 | Multi-device conflict timing + server-atomic update (MALI-022) | 2 devices + live Supabase | Concurrent edits on two devices produce exactly one conflict, resolvable both ways; ideally a `version`/`WHERE updated_at=base` guarded server update closes the client-side TOCTOU | A lost update slips through the read-then-write race | Client base-token compare is shipped; add a server-atomic conditional update (needs a migration + live test) |
