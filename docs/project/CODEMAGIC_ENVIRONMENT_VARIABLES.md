...
| `GOOGLE_SERVER_CLIENT_ID` | **android-release** (passed to all 3, gated only on Android) | NO (public config) | **Required for Android** | **MISSING — must be created** | `<project>-<hash>.apps.googleusercontent.com` (a **Web** OAuth client, not the Android one) | Google Cloud Console → project 881903820931 → Credentials → OAuth client → **Web application** | **EXTERNAL BLOCKED** | `android-release` **fails the build** (deliberate). Sign in with Apple is iOS-only and there is no guest path, so Google is Android's ONLY way in; shipping without it produces an app nobody can log into. iOS is unaffected and must NOT be gated on it. |
