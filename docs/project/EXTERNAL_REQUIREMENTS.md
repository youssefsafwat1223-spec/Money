...
| `app-ads.txt` | AdMob app verification | Serving to a newly configured app | ✅ **GENERATED AND VERIFIED LOCALLY 2026-09-04 — not yet deployed** | qirsh.site | `ADMOB_PUBLISHER_ID` | Publisher id supplied by the owner and verified. Build with `ADMOB_PUBLISHER_ID=pub-… python3 tools/build_site.py`, then deploy. **The emitter now lives in `build_site.py`** — the builder whose output is actually deployed. It previously existed only in `build_legal_site.py`, which writes `build/legal/`, a directory nothing publishes, so setting the id would have produced no live file. Logic is shared via `emit_app_ads_txt()`, not duplicated. The builder validates `pub-<16 digits>` and **refuses to write a placeholder**; no id is hardcoded in any builder. Pinned by `app/test/core/legal_urls_test.dart` (both guards proven non-vacuous). **Remaining: deploy** |
