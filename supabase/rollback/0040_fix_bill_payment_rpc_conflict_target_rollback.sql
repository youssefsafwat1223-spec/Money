-- Restores the exact 0039 function definition, including its broken
-- ON CONFLICT (user_id, local_id) clause (missing the WHERE local_id IS NOT NULL
-- predicate required by the partial unique index). Do not roll back unless a new,
-- unrelated regression is suspected to originate from 0040 — the pre-0040 version
-- fails 100% of the time with 42P10 on every call.
