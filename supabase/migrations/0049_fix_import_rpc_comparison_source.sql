...
begin
  ...
  for r in select value from jsonb_array_elements(coalesce(p_package->'accounts','[]'::jsonb)) loop
    v_is_default := coalesce(nullif(r->>'is_default','')::boolean,false);
    if p_mode = 'merge' then
      v_is_default := false;
    else
      if v_is_default then
        if v_default_assigned then
          v_is_default := false;
        else
    ...
    values(uid,r->>'record_id',r->>'name',upper(r->>'currency'),coalesce(nullif(r->>'type',''),'bank'),
      ...
      v_is_default,coalesce(nullif(r->>'sort_order','')::int,0),
  ...
  for r in select value from jsonb_array_elements(coalesce(p_package->'transactions','[]'::jsonb)) loop
    ...
    values(uid,'portable:'||(r->>'record_id'),nullif(r->>'account_record_id',''),
      ...
      when 'income' then 'income' when 'deposit' then 'income' when 'transfer' then 'transfer'
      else 'unknown' end,nullif(r->>'merchant',''),r->>'note',null,category_server_id,
      (r->>'occurred_at')::timestamptz,coalesce(nullif(r->>'source',''),'imported'),1,
      '{}'::jsonb,coalesce(nullif(r->>'status',''),'confirmed'),
      ...
      case coalesce(r->>'comparison_timestamp_source','') when 'sms_body' then 'sms_body' else 'received_at' end,null)
  ...
  for r in select value from jsonb_array_elements(coalesce(p_package->'budgets','[]'::jsonb)) loop
    ...
    values(uid,r->>'record_id',r->>'name',(r->>'amount')::numeric,upper(r->>'currency'),
      coalesce(nullif(r->>'type',''),'monthly'),(r->>'start_date')::date,(r->>'end_date')::date,
