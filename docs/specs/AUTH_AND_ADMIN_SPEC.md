...
- في الـ wireframes السابقة (docs/design/WIREFRAMES.md) أي إيموجي = **placeholder** يُستبدل بأيقونة مسمّاة. التعويض:
  - 🔥 streak → `icon/flame`
  - 🎯 هدف → `icon/target`
...
1. **docs/specs/PRODUCT_SPEC.md §7 (الخصوصية):** استبدال «لا سيرفر إطلاقاً» بصياغة القسم 2 هنا.
2. **docs/specs/PRODUCT_SPEC.md §17:** إضافة backend المصادقة كاستثناء معرّف (auth + metrics + rules)، مع تأكيد عدم رفع بيانات مالية.
3. **docs/design/WIREFRAMES.md:** إدراج Auth-1/Auth-2 بعد شاشة الخصوصية + إزالة كل الإيموجي واستبدالها بأيقونات مسمّاة.
...
3. ~~حذف الحساب: soft delete أم فوري؟~~ ✅ **محسوم: Soft Delete 30 يوماً** للهوية على السيرفر (قابل للاستعادة بالدخول)، والبيانات المالية المحلية تُمسح فوراً. انظر docs/specs/PRODUCT_SPEC.md §24.2.
