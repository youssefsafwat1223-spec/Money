...
- **Last updated:** **Phase 5 Batch 5 (backend, RLS, SECURITY DEFINER, metrics,
  ...
  (server-only authoritative writer) + adds a claim-before-award idempotency
  ledger (legacy path exactly-once per transaction). Client verified pull-only;
  engagement submission already bounded. `9fdd30e7`. Batch 6 not started.
