...
| ~~A4~~ | ~~No Android build/compile in any gate~~ — **CLOSED**: builds locally (§20.2) **and** an Android compile gate now runs in the canonical pre-release CI workflow (§20.3). Remote execution pending first push. | **CLOSED** |
...
| BACKUP | **READY** |
| CI | **READY_FLAG_OFF** — canonical gate **12 passed / 0 failed / 1 unavailable** (iOS packaging provenance only, not a pass). Android compile now gated pre-release (§20.3; **remote execution pending push**). Residual: Codemagic build workflows still bypass `ci_gates.sh` (C1) |
