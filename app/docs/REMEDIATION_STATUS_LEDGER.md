...
- **Last updated:** **Phase 6 Batch 4 closure #2 — CROSS-ISOLATE lease +
  ownership generation — 2026-08-06.** The in-memory gate could not coordinate the
  production background isolates; replaced with a filesystem lease (atomic
  maintenance-intent marker + a registry of per-secondary lease files, stale-
  recovered). `openSecondary` acquires a cross-isolate SHARED lease (refused
  during file-exclusive maintenance); `runExclusiveMaintenance(fileExclusive)`
  publishes intent + drains every secondary lease before proceeding; both
  production secondary paths pass `appSupportLeaseManager()`. Proven with a real
  `Isolate.spawn` test. Cross-isolate ownership generation (`OwnershipGuard`,
  reusing the Phase-2 owner UID) aborts an old-user background job before commit.
  MALI-069n + MALI-027 lifecycle tail → Code complete · Locally verified; native
  process timing + real-device SQLite contention pending. +15 more closure tests.
  analyze 0, full suite 1369, deno 76/0/2-ignored, node 29/0 (+27 skips), migration
