...
| Dependency | Purpose | Required for | Status | Environment | Secret name | Owner action |
|---|---|---|---|---|---|---|
| AdMob app IDs | SDK init | Any ad serving | **MISSING** — supply procedure documented | Build (Codemagic `admob` group) | `ADMOB_APP_ID_IOS`, `ADMOB_APP_ID_ANDROID` | Shape `ca-app-pub-<16 digits>~<10 digits>` (**tilde**). See `ADMOB_PRODUCTION_READINESS.md` §6 for the exact supply step. Android reads it from the **environment** in `build.gradle.kts` *and* as a dart-define; iOS needs it written to `ios/Flutter/AdMob.xcconfig`, which `codemagic.yaml` already does |
| AdMob interstitial units | Report-export ad | Interstitial revenue | **MISSING** — supply procedure documented | Build (Codemagic `admob` group) | `ADMOB_INTERSTITIAL_IOS/_ANDROID` | Create two units. Shape `ca-app-pub-<16 digits>/<10 digits>` (**slash**, not tilde) |
| AdMob banner units | Transactions banner | Banner revenue | **MISSING** — supply procedure documented | Build (Codemagic `admob` group) | `ADMOB_BANNER_IOS/_ANDROID` | Create two units. Shape `ca-app-pub-<16 digits>/<10 digits>` (**slash**). Wired into the release pipeline on 2026-09-04 — they were previously not passed at all |
