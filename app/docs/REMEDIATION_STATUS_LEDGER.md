...
| MALI-029 | Med | C | P7 | **Partial · Locally verified** | **P7-B2.** Scoped invalidation DONE: `AppDatabase.tableWriteStream` emits each write's target table; `scopedRevisionProvider(family)` + `financialRevisionProvider` (exclude-operational) replace the global tick for reports/budgets/transaction-list/dashboard; unrelated write→0 rebuilds, relevant/display-dep→exactly 1, 100-burst→≤2, operational→0 (`scoped_invalidation_test.dart`). Pull-batching STARTED: CaptureSyncService prefetches accounts once/run (was getAll() per row) — 8 captures→1 read. **Remaining:** batch the other ~14 pull/backfill services + sync cadence (offline pause/backoff). See `PHASE_7_PERFORMANCE_CONTRACT.md`. |
