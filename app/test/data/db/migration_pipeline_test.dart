...
void main() {
  ...
  Future<void> insertMarker(AppDatabase db) => db.customStatement(
  ...
  Future<void> assertFullyMigrated(AppDatabase db) async {
    expect(await userVersion(db), 27, reason: 'must reach target version');
  ...
  test('a newer-than-app schema FAILS CLOSED with a distinguishable exception',
    ...
    expect((caught! as UnsupportedDatabaseVersionException).databaseVersion,
        999);
    expect((caught as UnsupportedDatabaseVersionException).supportedVersion, 27);
  ...
  test('fixture A — empty database (user_version 0) builds forward to v27',
      () async {
